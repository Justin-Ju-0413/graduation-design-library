...
def build_deck():
    ...
    slides.append(("Board bring-up evidence", "The hello_e203 run closed the CPU boot path with UART and ILA evidence.", ["MROM boot redirects execution to ITCM.", "UART proves firmware-level execution.", "ILA confirms PC activity and board-level reset/clock health."], "fig4_2_ila_pc_trace.png", "Use this slide to explain how ILA made the board bring-up observable."))
    slides.append(("CNN board result", "The CNN v1 demo passed on the FPGA: hardware output matched software and expected values.", [], "fig4_4_uart_cnn_v1.png", "State the board result plainly: SW output, HW output, expected output all match; speedup is 5.282x."))
    slides.append(("Performance and fit", "The accelerator reaches the target convolution speedup while fitting comfortably in the A7-100T.", [], "fig4_6_speedup_bar.png", "Start with speedup, then mention resource headroom and why end-to-end inference is still limited by software FC layers."))
