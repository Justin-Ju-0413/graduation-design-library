...
\caption{System architecture of the E203 SoC with the CNN NICE accelerator, showing the register-based NICE path and the standard AHB memory/peripheral path.}
\label{fig:3_1}
\end{figure}
...
\caption{RISC-V custom0 instruction format used by the accelerator; bits [14:12] carry the NICE xd/xs1/xs2 flags and funct7 selects the accelerator operation.}
\label{fig:3_2}
\end{figure}
...
\begin{table}[htbp]
\centering\small
\caption{NICE custom instruction set used by the CNN accelerator firmware. WLOAD and DLOAD use rs2 as a vector-bank index, which is why the decoder-side rs2 capture fix is required for index 0.}
\label{tab:3_3}
\resizebox{\textwidth}{!}{%
\begin{tabular}{lllll}
\hline
Instruction & Encoding (hex) & Fields & Operands & Description \\ \hline\hline
CFG   & \texttt{0x0A00100B} & f7=5, xd=0, xs1=1, xs2=0 & rs1[0]=ReLU en & Configure accelerator \\ \hline
CLEAR & \texttt{0x0800000B} & f7=4, xd=0, xs1=0, xs2=0 & none & Clear PE accumulators \\ \hline
WLOAD & \texttt{0x0001800B} & f7=0, xd=0, xs1=1, xs2=1 & rs1=data, rs2=idx & Load weight column \\ \hline
DLOAD & \texttt{0x0201800B} & f7=1, xd=0, xs1=1, xs2=1 & rs1=data, rs2=idx & Load activation row \\ \hline
COMP  & \texttt{0x0400000B} & f7=2, xd=0, xs1=0, xs2=0 & none & Trigger MAC computation \\ \hline
RSTAT & \texttt{0x0600250B} & f7=3, xd=1, xs1=0, xs2=0 & rd=result & Read accumulator result \\ \hline
\end{tabular}
}
\end{table}
...
The COMP instruction implements a guard condition: the accelerator asserts en\_pe only after all four weight-load and data-load operations have completed. The condition is tracked by w\_loaded\_mask and d\_loaded\_mask, preventing undefined behavior from partial data loading.

A subtle integration constraint exists in the E203 core's instruction decoder~\cite{NUCLEI_E203,NUCLEI_NICE}. The decoder uses the rs2 register field for two purposes simultaneously: as a register file read address and as a qualifier for whether the rs2 operand is needed at all. When the rs2 field encodes register x0 (5'b00000), the decoder treats this as ``rs2 not required'' and gates the IFU's rs2 index register update (ir\_rs2idx\_ena~=~0). For standard RISC-V instructions this is correct behavior, since x0 is hardwired to zero and reading it is unnecessary. However, NICE instructions that repurpose the rs2 field as a data operand—such as WLOAD and DLOAD, which use rs2 as a vector bank index (0--3)—are affected: when the index is 0, the rs2 field encodes 5'b00000, causing the decoder to suppress the rs2 read and potentially deliver stale data to the accelerator. The corrective measure applied in this design is to bypass the x0-gating for NICE-flagged instructions in the decoder's rv32\_need\_rs2 expression, ensuring the rs2 index is always captured regardless of its encoded value. This fix is discussed in detail as a bring-up case study in Chapter~4.
...
\caption{Processing element microarchitecture: each PE performs one signed INT8 multiply and accumulates into an INT32 output register.}
\label{fig:3_3}
\end{figure}
...
\caption{4x4 PE array organization. WLOAD broadcasts weight columns, DLOAD broadcasts activation rows, and COMP triggers parallel output-stationary accumulation.}
\label{fig:3_4}
\end{figure}
...
The CNN accelerator is integrated into the E203 SoC at the subsystem level. The E203 NICE subsystem wrapper provided by Nuclei instantiates cnn\_nice\_core and connects the request/response signals between the processor core and the accelerator.
...
\caption{Reproducible FPGA build pipeline from firmware image generation through Vivado implementation, board programming, and UART/ILA evidence capture.}
\label{fig:3_6}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig3_7_verification_chain.png}
\caption{Verification chain used for the thesis and defense: RTL simulation, full-SoC simulation, hello\_e203 board validation, CNN board demo, and the NICE rs2 fix regression.}
\label{fig:3_7}
\end{figure}
