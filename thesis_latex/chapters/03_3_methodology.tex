...
The system architecture integrates a RISC-V E203 Hummingbird v2 processor core with a custom CNN accelerator through the NICE coprocessor interface. Figure 3.1 shows the top-level block diagram of the complete SoC.
...
The E203 core is a 32-bit, 2-stage in-order RISC-V processor implementing the RV32IMAC instruction set. It provides a complete memory subsystem including an instruction tightly-coupled memory (ITCM) mapped at 0x8000\_0000 and a data tightly-coupled memory (DTCM) mapped at 0x9000\_0000. Standard peripherals—the Core Local Interruptor (CLINT), Platform-Level Interrupt Controller (PLIC), UART0, and GPIO—are connected through the system bus fabric.
...
Table 3.1 presents the address map of the system.
...
DTCM & 0x9000\_0000 - & 32-bit & Data memory (64 KB) \\ \hline
 & 0x9000\_FFFF &  &  \\ \hline
CLINT & 0x0200\_0000 & 32-bit & Core local interruptor \\ \hline
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig3_2_instruction_format.png}
...
\hline
Instruction & funct7 & xd & xs1 & xs2 & Encoding (hex) & Description \\ \hline\hline
CFG & 5 & 0 & 1 & 0 & 0x0A00100B & Configure: rs1[0] enables ReLU \\ \hline
CLEAR & 4 & 0 & 0 & 0 & 0x0800000B & Clear all PE accumulators and masks \\ \hline
WLOAD & 0 & 0 & 1 & 1 & 0x0001800B & Load weight column: rs1=data, rs2=idx \\ \hline
DLOAD & 1 & 0 & 1 & 1 & 0x0201800B & Load activation row: rs1=data, rs2=idx \\ \hline
COMP & 2 & 0 & 0 & 0 & 0x0400000B & Trigger MAC computation on all PEs \\ \hline
RSTAT & 3 & 1 & 0 & 0 & 0x0600250B & Read accumulator result into rd \\ \hline
\end{tabular}
\end{table}
...
The decoder includes two error-checking mechanisms. First, if the instruction opcode is not 0x0B (custom0), the accelerator reports an error via nice\_rsp\_err. Second, for WLOAD and DLOAD instructions, the rs2 index is validated: only the lower two bits may be nonzero, ensuring the index is in the range \cite{ASANOVIC_HOTCHIPS}.
...
Figure 3.3 shows the PE datapath.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig3_3_pe_microarchitecture.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig3_4_pe_array.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig3_5_packed_format.png}
...
The simulation validates: 1. Normal path: CLEAR, four WLOADs, four DLOADs, COMP, RSTAT — the expected result is 320 for a known test vector. 2. Error paths: COMP before loading all data, RSTAT before COMP. 3. Boundary conditions: Negative INT8 values (-128, -1), maximum positive values (127). 4. Reset behavior: Accumulators cleared after reset.
