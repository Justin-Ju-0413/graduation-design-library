...
heartbeat\_mmcm\_sysclk\_ila & 13.887 & 0.061 & Clean \\ \hline
...
cnn\_sysclk\_ila & 12.468 & 0.057 & Clean \\ \hline
\end{tabular}
\end{table}

\section{hello\_e203 Board Validation}\label{sec:4_3}

The Davinci Pro A7-100T FPGA development board (Figure~\ref{fig:fpga_board}), featuring a Xilinx Artix-7 xc7a100tfgg484-2 FPGA, serves as the hardware validation platform. The board is configured via JTAG with the E203 SoC and NICE CNN accelerator bitstream.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_fpga_board.jpg}
\caption{Davinci Pro A7-100T FPGA development board running the LeNet-5 CNN inference demo.}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig4_1_ila_pc_trace.png}
...
Root Cause 2: ITCM BRAM Initialization Format (64-bit) The E203 ITCM uses 64-bit wide block RAM (\texttt{E203\_ITCM\_DATA\_WIDTH}=64). The \texttt{riscv64-unknown-elf-objcopy}~\mbox{\texttt{-O verilog}} command produces byte-level hex files (e.g., ``17 01 01 10’’), but Vivado’s \texttt{\$readmemh} interprets each whitespace-separated token as a full memory word. For 64-bit ITCM, each word requires 16 hex digits (two 32-bit instructions packed together with little-endian byte ordering). A custom PowerShell script was developed to convert byte-level hex to the required word-level format.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig4_2_ila_nice_activity.png}
\caption{ILA capture showing CPU execution in the CNN program code region with active memory references. NICE CSR and handshake probes show idle state, indicating the capture window precedes the NICE instruction burst.}
\label{fig:4_2}
\end{figure}
...
The fix was verified at three levels. First, the light-weight NICE simulation testbench (tb\_e203\_nice\_light) was run on the patched RTL and passed all 7 test cases, including the normal\_path case with RSTAT~=~320 and the invalid\_index error-detection case. Second, the full FPGA bitstream was rebuilt (cnn\_sysclk\_ila mode) and achieved timing closure with WNS~=~13.512~ns and zero unrouted nets. Third, the CNN v1 accelerator demo was executed on the Davinci Pro A7-100T board, producing UART output that confirmed hardware-software agreement (HW output: 12~23~0~19, matching both the software reference and expected values) and the same 5.282$\times$ speedup (287 vs.\ 1516~cycles) measured before the fix. The ILA re-capture confirmed stable NICE handshake behavior with no regressions~\cite{XILINX_UG908}.
...
\subsubsection{FPGA Deployment Status}

On the FPGA platform, the NICE instruction execution sequence was confirmed via ILA probe traces: the CPU successfully executes the complete NICE instruction flow (CLEAR, WLOAD$\times$4, DLOAD$\times$4, COMP, RSTAT) and reaches the program completion state. The full LeNet-5 inference pipeline was deployed and tested on 10 MNIST test images.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_uart_output.png}
\caption[UART terminal output from the LeNet-5 MNIST inference demo]{UART terminal output from the LeNet-5 MNIST inference demo running on the FPGA. The system correctly classified all 10 of 10 test images (100\%), confirming end-to-end functionality of the NICE-accelerated inference pipeline with INT8-quantized weights achieving 98.34\% accuracy on the full MNIST test set (10,000 images). Two key corrections enabled this result: (1) fixing the Conv2 multi-channel ReLU timing to apply activation after channel summation rather than per-channel, and (2) using per-layer calibrated rescaling factors (generated by the Python quantization toolchain) for Conv2-to-FC1 input rescaling and FC layer output right-shift operations.}
\label{fig:uart_output}
\end{figure}

The measured 98.34\% inference accuracy represents only a 1.7\% degradation from the FP32 baseline~\cite{LENET} (98.5\%), well within the project's 5\% accuracy loss target and comparable to state-of-the-art INT8 quantization results~\cite{JACOB_QUANT}. This result was achieved after correcting two implementation issues. First, the Conv2 multi-channel convolution originally applied ReLU on a per-input-channel basis before summing contributions across channels; moving the ReLU to after channel summation restores negative-value cancellation across channels---a critical property for multi-channel convolution. Second, per-layer calibrated rescaling factors (generated by a Python quantization calibration script) replaced a set of uniform hard-coded shift values, correcting a 2--4$\times$ magnitude mismatch in FC layer outputs. The convolutional layers (Conv1 and Conv2), executed entirely in INT8 via the NICE accelerator, produced numerically correct results verified by per-pixel CPU reference equivalence checks embedded in the firmware. The FC layers perform INT8-weight $\times$ INT32-activation multiplication with INT64 accumulation and calibrated right-shift rescaling, achieving accuracy within 1.7\% of the FP32 reference.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{figures/fig4_3_speedup_bar.png}
...
The FPGA resource utilization for the bootvec\_sysclk\_ila build is summarized in Table~\ref{tab:4_6} and Figure~\ref{fig:4_4} (post-implementation, xc7a100tfgg484-2).


\begin{table}[htbp]
\centering
\caption[FPGA resource utilization for the complete SoC]{FPGA resource utilization for the complete SoC (post-implementation, bootvec\_sysclk\_ila build)}
\label{tab:4_6}
\begin{tabular}{lcccc}
\hline
Resource & Used & Available & Utilization (\%) \\ \hline\hline
LUT & 13,187 & 63,400 & 20.8 \\ \hline
LUTRAM & 2,843 & 19,200 & 14.8 \\ \hline
FF & 12,807 & 126,800 & 10.1 \\ \hline
BRAM & 36 & 135 & 26.7 \\ \hline
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.70\textwidth]{figures/fig4_4_resource_pie.png}
\caption{FPGA resource utilization distribution for the complete E203 SoC with CNN NICE accelerator (post-placement).}
\label{fig:4_4}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{figures/fig4_5_utilization.png}
\caption{FPGA resource utilization bar chart for the complete E203 SoC with CNN NICE accelerator (post-placement).}
\label{fig:4_5}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.80\textwidth]{figures/fig4_6_timing.png}
