...
To validate the full inference pipeline, a complete LeNet-5 network (Conv1 5$\times$5 1$\to$6, MaxPool 2$\times$2, Conv2 5$\times$5 6$\to$16, MaxPool 2$\times$2, FC1 256$\to$120, FC2 120$\to$84, FC3 84$\to$10) was implemented in C with the NICE custom instruction driver. The INT8-quantized weights (98.8\% FP32 baseline accuracy on MNIST, trained with input normalization aligned to the C-side preprocessing) were compiled into the firmware image.
...
\caption{UART terminal output from the LeNet-5 MNIST inference demo running on the FPGA. The system correctly classified 7 of 10 test images (70\% accuracy), confirming end-to-end functionality of the NICE-accelerated inference pipeline. Classification failures on images 6, 7, and 9 are attributed to INT8 re-quantization precision loss in the Conv2-to-FC1 transition, where fixed-bit-width rescaling of INT32 accumulator outputs back to the INT8 input range required by subsequent layers introduces cumulative error.}
\label{fig:uart_output}
\end{figure}

The 70\% inference accuracy represents a 28.5\% degradation from the FP32 baseline (98.5\%). This accuracy gap is primarily attributable to the INT8 re-quantization step between the Conv2 output (INT32) and the FC1 input (INT8), where fixed-rshift rescaling introduces cumulative precision loss across the three fully-connected layers. The Conv1 and Conv2 layers, executed entirely in INT8 via the NICE accelerator, demonstrated functional correctness verified by per-pixel equivalence checks against a CPU reference implementation embedded in the firmware. The fully-connected layers, performing INT8-weight $\times$ INT32-activation multiplication with INT64 accumulation and fixed-point rescaling, account for the majority of the observed accuracy gap. This is a known challenge in INT8-only inference pipelines without per-layer calibration, and is documented as a limitation for future work.
